...
export function splitTheBill(x: {[k: string]: number}): {[k: string]: number} {
    const totalBill = Object.values(x).reduce((a, b) => a + b, 0);
    const numberOfPeople = Object.values(x).length;
    const share = totalBill / numberOfPeople;

    const result: {[k: string]: number} = {};
    for (const person in x) {
        result[person] = +(x[person] - share).toFixed(2);
    }
    return result;
}
