...
// ✅ SOLUTION
export function topSecret(s: string): string {
  let decrypted = '';

  for (const char of s) {
    if (char >= 'A' && char <= 'Z') {
      decrypted += String.fromCharCode((char.charCodeAt(0) - 3 < 65)
        ? char.charCodeAt(0) + 23
        : char.charCodeAt(0) - 3);
    } else if (char >= 'a' && char <= 'z') {
      decrypted += String.fromCharCode((char.charCodeAt(0) - 3 < 97)
        ? char.charCodeAt(0) + 23
        : char.charCodeAt(0) - 3);
    } else {
      decrypted += char;
